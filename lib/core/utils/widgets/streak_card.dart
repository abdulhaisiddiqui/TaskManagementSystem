...
class StreakCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          Text("$currentStreak", style: TextStyle(fontSize: 48, color: Colors.white)),
          SizedBox(width: 16),
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text("DAY STREAK", style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.white)),
              Text("Best: $longestStreak days", style: TextStyle(color: Colors.white70)),
            ],
          ),
          Spacer(),
          if (currentStreak >= 7)
